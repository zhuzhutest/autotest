...
MAIL_PICTURE_FUNC = {"TEST_COMPANY" => ["test_managed"], "TEST_IOS_NEWS" => ["test_news_search"]}
class RUN_TESTSUIT
    @@all_suit_info = ""
    @@all_case_info = ""
    @@all_stream_info = ""
    ...
    def initialize(brow, site, user, password, filename)
        if  brow == nil && site == nil && user == nil && password == nil
    ...
    def run_case(className, funcName)
        ...
        cs = Case_stats.new
        begin
            Dir[File.dirname(__FILE__)+'/case/api_case/*.rb'].each {|file|
    ...
    def write_log
        ...
        @f_cur = File.new(File.dirname(__FILE__)+'/case/log/last_api_messages.log', "w+")
        @f.puts "------------------------------------------------------------------------------------------------------------"
        ...
        if @@is_mail_send
            ...
            #         }
            # }
            KR::send_mail(run_result, FROM_MAIL, TO_ALL_MAIL)
            @@is_mail_send = false
        end
        reinit_data
        @@run_times += 1
